...
async fn generate_test_event(config: &ClientConfig, kind: GroupEventKind) -> Result<EventBuilder> {
    ...
    let content = match kind {
        ...
        GroupEventKind::CreateInvite => {
            let invite_code = config.invite_code.as_ref().ok_or_else(|| anyhow::anyhow!("Invite code is required for CreateInvite event"))?;
            json!({
                "type": "invite",
    ...
    if let Some(group_id) = &config.group_id {
        ...
        match kind {
            ...
            GroupEventKind::RemoveUser => {
                ...
                builder = builder.tag(Tag::custom(TagKind::p(), [config.keys.public_key().to_string()]));
            },
...
async fn create_group(config: &ClientConfig) -> Result<String> {
    ...
    info!("Group metadata update sent to: {:?}", metadata_output.success);
    if !metadata_output.failed.is_empty() {
        info!("Group metadata update failed for: {:?}", metadata_output.failed);
    }
    ...
    let Output { val: verify_sub_id, .. } = verify_client.subscribe(vec![verify_filter.clone()], None).await?;
                ...
                async move {
                    if let RelayPoolNotification::Event { subscription_id, event, .. } = notification {
                        if subscription_id == sub_id {
                            info!("Received group state verification event: {} from pubkey {}", event.id, event.pubkey);
    ...
    for i in 0..5 {
        ...
        match tokio::time::timeout(Duration::from_secs(5u64.pow(i)), rx.recv()).await {
            Ok(Some(event)) => {
                info!("Received group state event: {} from pubkey {}", event.id, event.pubkey);
...
async fn main() -> Result<()> {
    ...
    let mut admin_config = ClientConfig::new(args.url.clone(), admin_keys.clone(), ClientRole::Admin);
    ...
    let Output { val: _verify_sub_id, .. } = verify_client.subscribe(vec![group_filter], None).await?;
    ...
    let invite_builder = generate_test_event(&admin_config_with_invite, GroupEventKind::CreateInvite).await?;
    ...
    let Output { val: verify_sub_id, .. } = invite_client.subscribe(vec![verify_filter], None).await?;
    ...
    let Output { val: invite_sub_id, .. } = invite_client.subscribe(vec![invite_filter], None).await?;
    ...
    for i in 0..5 {
        info!("Waiting for invite creation confirmation, attempt {}", i + 1);
        match tokio::time::timeout(Duration::from_secs(2u64.pow(i)), rx.recv()).await {
            Ok(Some(_)) => {
    ...
    for i in 0..num_invite_clients {
        ...
        let mut config = ClientConfig::new(args.url.clone(), Keys::generate(), ClientRole::NonMember);
    ...
    loop {
...

        {
            ...
            if !output.failed.is_empty() {
                ...
                error!("Failed to send {:?} event to relays: {:?}", event_kind, output.failed);
            }

            if !output.success.is_empty() {
                info!("Successfully sent {:?} event to relays: {:?}", event_kind, output.success);
            }
        }
