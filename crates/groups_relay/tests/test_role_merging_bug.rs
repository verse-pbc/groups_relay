...
async fn test_group_creator_remains_admin_after_members_list() {
    ...
    assert!(group.is_admin(&creator_keys.public_key()), "Creator should be admin immediately after group creation");
    ...
    assert!(loaded_group.is_admin(&creator_keys.public_key()), "Creator should be admin after loading 39001");
    
    // Then load members list - this is where the bug was happening
    loaded_group.load_members_from_event(&members_event).unwrap();
    ...
    assert!(loaded_group.is_member(&creator_keys.public_key()), "Creator should be a member");
    assert!(loaded_group.is_member(&other_member_keys.public_key()), "Other user should be a member");
    
    // Verify creator has both Admin and Member roles
    let creator_member = loaded_group.members.get(&creator_keys.public_key()).unwrap();
    assert!(creator_member.roles.contains(&GroupRole::Admin), "Creator should have Admin role");
    assert!(creator_member.roles.contains(&GroupRole::Member), "Creator should have Member role");
    
    // Verify other member only has Member role
    let other_member = loaded_group.members.get(&other_member_keys.public_key()).unwrap();
    assert!(!other_member.roles.contains(&GroupRole::Admin), "Other member should NOT have Admin role");
    assert!(other_member.roles.contains(&GroupRole::Member), "Other member should have Member role");
}
...
async fn test_multiple_admins_preserved_after_members_event() {
    ...
    .tag(Tag::custom(TagKind::d(), vec![group_id]))
    .tag(Tag::custom(TagKind::p(), vec![admin1_keys.public_key().to_string(), "Admin".to_string()]))
    .tag(Tag::custom(TagKind::p(), vec![admin2_keys.public_key().to_string(), "Admin".to_string()]))
    .sign_with_keys(&relay_keys)
    .unwrap();
    ...
    .tag(Tag::custom(TagKind::d(), vec![group_id]))
    .tag(Tag::custom(TagKind::p(), vec![admin1_keys.public_key().to_string()]))
    .tag(Tag::custom(TagKind::p(), vec![admin2_keys.public_key().to_string()]))
    .tag(Tag::custom(TagKind::p(), vec![member_keys.public_key().to_string()]))
    .sign_with_keys(&relay_keys)
    .unwrap();
    ...
    assert!(group.is_admin(&admin1_keys.public_key()), "Admin1 should still be admin");
    assert!(group.is_admin(&admin2_keys.public_key()), "Admin2 should still be admin");
    assert!(!group.is_admin(&member_keys.public_key()), "Regular member should not be admin");
    
    // Verify everyone is a member
    assert!(group.is_member(&admin1_keys.public_key()), "Admin1 should be member");
    assert!(group.is_member(&admin2_keys.public_key()), "Admin2 should be member");
    assert!(group.is_member(&member_keys.public_key()), "Regular member should be member");
}
...
async fn test_admins_event_only_contains_admin_roles() {
    ...
    // Check the tags
    let p_tags: Vec<_> = admins_event.tags
        .iter()
    ...
    assert_eq!(tag_slice.len(), 3, "Should only have p, pubkey, and Admin role");
    assert!(!tag_slice.contains(&"Member".to_string()), "Admin tag should not contain Member role");
}