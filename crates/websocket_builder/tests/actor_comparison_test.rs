...
use websocket_builder::{
    ActorWebSocketBuilder, InboundContext, MessageConverter, Middleware, SendMessage,
    StateFactory, WebSocketBuilder,
};
...
async fn compare_head_of_line_blocking() {
    ...
    let single_loop_result = test_single_loop_architecture(
        processing_delay,
        message_count,
        channel_size,
    ).await;
    ...
    let actor_based_result = test_actor_based_architecture(
        processing_delay,
        message_count,
        channel_size,
    ).await;
    ...
    println!("  Messages processed: {}", single_loop_result.messages_processed);
    println!("  Avg response latency: {:?}", single_loop_result.avg_response_latency);
    println!("  Max response latency: {:?}", single_loop_result.max_response_latency);
    
    println!("\nActor-Based Architecture:");
    println!("  Total time: {:?}", actor_based_result.total_time);
    println!("  Messages processed: {}", actor_based_result.messages_processed);
    println!("  Avg response latency: {:?}", actor_based_result.avg_response_latency);
    println!("  Max response latency: {:?}", actor_based_result.max_response_latency);
    
    println!("\nIMPROVEMENT METRICS:");
    let time_improvement = single_loop_result.total_time.as_secs_f64() 
        / actor_based_result.total_time.as_secs_f64();
    ...
    println!("  Response latency improvement: {:.2}x lower", latency_improvement);
    println!("  Head-of-line blocking eliminated: {}", 
        if latency_improvement > 5.0 { "YES ✓" } else { "PARTIAL" });
}
...
async fn compare_concurrent_load() {
    ...
    println!("Testing with {} connections, {} messages each", 
